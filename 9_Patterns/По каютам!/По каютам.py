...
class LoginForm(FlaskForm):
    username_1 = StringField('Id астронафта', validators=[DataRequired()])
    password_1 = PasswordField('Пароль астронафта', validators=[DataRequired()])
...
def login():
    ...
    data['form'] = form
    data['path_to_emblem'] = url_for('static', filename='img/emblem.png')

    return render_template('login.html', **data)


@app.route('/distribution/<path:list>')
def distribution(list):
    list = list.split('/')
    return render_template('cabin.html', people=list)
